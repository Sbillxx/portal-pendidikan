...
const TableIklan = ({ data, refresher, type }) => {
  ...
  return (
    <>
      ...
      <div className="table-container">
        <Table striped>
          <Table.Head>
            ...
            <Table.HeadCell>Ukuran</Table.HeadCell>
          ...
          <Table.Body className="divide-y">
            {[...data].reverse().map((ele, index) => (
              <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                ...
                <Table.Cell>{ele.sumber}</Table.Cell>
                <Table.Cell>{ele.ukuran}</Table.Cell>
                <Table.Cell>{ele.tanggal}</Table.Cell>

                <Table.Cell>
                  <div className="flex items-center gap-4">
                    <Link
                      to={`/admincms/iklan/${ele._id}`}
      ...
      <DeleteModal
        ...
        namaItem={judulItem}
        deleteFunction={`${base_url}/iklan/delete/${idItem}`}
        refresher={refresher}
      />
      {/* open modal */}
    </>
